<template>
  <!-- 数据超市-首页 -->
  <div class="data_mart_home" v-loading="loading">
    <!-- 用户未登录 -->
    <div v-if="!userInfo.name" class="not_login">
      <div>
        <el-image
          class="py-5"
          :src="login_hint"
        ...
        ></el-image>
        <div class="title">您必须登录以访问该内容</div>
        <div class="hint">
          请在下方选择登录的方式，您可以通过个人账户或机构（IP）登入系统
        </div>
        <div class="button">
          <el-button class="white_button" @click="IPLogin"
            >机构用户（IP）登录</el-button
          >
    ...
    <div v-else class="home">
      <el-container>
        ...
        <el-header height="60px">
          ...
          <el-button
            class="text_button"
            style="color: #5051f9; margin-left: 0; width: 170px"
            type="text"
            @click="jumpRouter('/data_mart/database')"
          >
            获取全部模块
          </el-button>
          <!-- 筛选 -->
            ...
            <el-container style="height: 500px">
              ...
              <el-footer height="50px" style="padding: 10px 20px 0">
                <span class="filter_footer">
                  ...
                  <el-button class="confirm" @click="filterPart"
                    >筛 选</el-button
                  >
                </span>
              </el-footer>
        ...
        <el-main class="main scrollbar">
          <!-- 自定义组件 -->
          <page-element @relayout="relayout" />
          <!-- 热点模块 -->
          <div v-if="partList && partList.length > 0" id="hot_part">
            <h1>
              热点模块
              <div></div>
            </h1>
            ...
            <div class="part_list">
              ...
              >
                ...
                <div class="part_info">
                  ...
                  <div
                    :style="
                      part.table && part.table.length > 1
                        ? ''
                        : 'visibility: hidden'
                    "
                  >
...
export default {
  ...
  methods: {
    ...
    getDatabaseTree() {
      ...
      this.$http
        ...
        .then((data) => {
          ...
          // 获取成功，处理数据
          if (res.status === 0) {
            this.databaseTree.push({
              name: "全部",
              code: "all",
              children: [],
            });
            this.expandedKeys.push(this.databaseTree[0].code);
            this.checkedKeys.push(this.databaseTree[0].code);
              ...
              res.data.forEach((a) => {
                ...
                if (a.children && a.children.length > 0)
                  a.children.forEach((b) => {
                    ...
                    this.checkedKeys.push(b.code);
                    database.children.push(b_database);
                  });
                this.databaseTree[0].children.push(database);
              });
            this.tempDatabaseTree = JSON.parse(
              JSON.stringify(this.databaseTree)
            );
            this.tempCheckedKeys = JSON.parse(JSON.stringify(this.checkedKeys));
          }
        })
        .catch(() => {})
        .finally(() => {});
    ...
    resetTree() {
      this.filterText = "";
      this.checkedKeys = JSON.parse(JSON.stringify(this.tempCheckedKeys));
    ...
    getPartList() {
      ...
      // 调用接口
      this.$http
        .get(process.env.API_ROOT + "/market/getShowPart", {
          params: {
        ...
        .then((data) => {
          ...
          if (res.status === 0) {
            ...
            if (res.data && res.data.length > 0)
              res.data.forEach((item) => {
                item.tags = Array.isArray(JSON.parse(item.tags))
                  ? JSON.parse(item.tags)
                  : [];
                for (let i = 0; i < item.table.length; i++)
                  if (item.table[i].tags && item.table[i].tags.length > 0) {
                    item.tags.push(item.table[i].tags[0].tagName);
                    break;
                  }
                for (let i = item.tags.length - 1; i >= 0; i--)
                  if (!item.tags[i]) item.tags.splice(i, 1);
              });
            res.data.sort((a, b) => b.tags.length - a.tags.length);
    ...
    relayout() {
      let interval = setInterval(() => {
        if (!this.loading) {
          const dom01 = document.getElementsByClassName("page_element")[0];
          if (dom01) dom01.style.minHeight = "auto";
          if (this.partList && this.partList.length > 0) {
            this.$nextTick(() => {
              const dom02 = document.getElementsByClassName("my-element")[0];
              const dom03 = document.getElementById("hot_part");
              if (dom03) {
                if (dom02) dom03.style.padding = dom02.style.padding;
