<template>
  ...
  <div class="about">
    <el-tabs tab-position="left"
             ...
             @tab-click="jump">
      <el-tab-pane v-for="(items, i) in menuList"
                   :key="i"
                   :name="items.name"
                   :myData="items">
        <span slot="label" class="not-selected"><i :class="items.icon + ' icon'"></i> {{ items.label }}</span>
        <div class="tabs_body">
          <el-tabs v-if="items.name === '0' && activeName === '0'" v-model="items.activeName"
                   class="tab"
                   :before-leave="beforeLeave_1"
                   @tab-click="jump_2">
            <el-tab-pane v-for="(item, j) in items.children"
                         :key="j"
                         :name="item.router"
                         :myData="item.router">
              <span slot="label" class="not-selected">{{ item.name }}</span>
              <div class="tab_body scrollbar">
                <router-view v-if="item.router === items.activeName"
                             style="height: 100%"
                             :pageName="item.name"
                             @IPLogin="IPLogin"/>
              </div>
          ...
          <router-view v-else-if="items.name === activeName"
                       style="height: 100%"
                       class="scrollbar"
                       @IPLogin="IPLogin"/>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      init() {
        ...
        if (this.menuList[1].path === path)
          this.activeName = '1';
        else if (this.menuList[2].path === path)
          this.activeName = '2';
        else {
          this.activeName = '0';
      ...
      // 获取菜单路由
      getMenuList() {
        this.menuList[0].children = this.$store.getters.getMenuList_1;
